...
df <- data.table::as.data.table(cbind(brand_a, brand_b))
melted <- data.table::melt(df)
...
cis[, brand := stringr::str_extract(regex("Brand.."), labels)]


d <- ggplot(data = melted) + aes(x = value, fill = variable, alpha = 0.5) + geom_density() + geom_vline(data = ci_mean_a, aes(xintercept= vals), color="blue", size=0.5) 
