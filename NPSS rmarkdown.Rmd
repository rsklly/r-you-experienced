...
By being able to transform the raw scores into something that can be computed as a mean, then we can also compute standard deviation, and therefore confidence intervals and run hypothesis tests. Our survey NPS can be seen as a sample mean, with a true It is here that our true troubles begin. 
...
Our survey sample NPS statistic is 10%, but 95% confidence intervals suggest that the _true population NPS statistic_ (the NPS metric if you were to ask the **entire addressable market** instead of a survey sample) is anywhere between -6% and ~28%!
...
cis <- as.data.table(rbind(ci_mean_a, ci_mean_b))
cis[, brand := str_extract(regex("Brand.."), labels)]


d <- ggplot(data = melted) + aes(x = value, fill = variable, alpha = 0.5) + 
  geom_density() + geom_vline(data = c(ci_mean_a, aes(xintercept= vals), color="blue", size=0.5) + geom_text(data = ci_mean_a, aes(xintercept= vals, label = labels))
